        ...
        do {
            ...
            if result.isMatch, let personId = result.personId {
                if let person = store.getLovedOne(byId: personId) {
                    ...
                    // Play audio reminder
                    audioService.speakRecognitionReminder(for: person, confidence: Double(result.similarity))
                } else {
                    status = .enrolled
                }
            } else {
    ...
    private func tryOfflineRecognition(image: UIImage) async {
        ...
        if let offlineResult = await offlineCache.matchOffline(image: image) {
            ...
            if let person = store.getLovedOne(byId: offlineResult.personId) {
                ...
                // Play audio reminder
                audioService.speakRecognitionReminder(for: person, confidence: offlineResult.similarity, isOffline: true)
            } else {
                faceDetectionManager.recognitionCompleted()
                status = .enrolled
            }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            HStack(alignment: .top, spacing: 14) {
                ...
                Button(action: {
                    audioService.speakRecognitionReminder(for: person, confidence: 100, isOffline: wasOffline)
                }) {
                    ZStack {
                        Circle()
