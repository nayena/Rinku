...
struct ProfileView: View {
    ...
    private var medicationSubtitle: String {
        let count = MedicationService.shared.entries.count
        if count == 0 { return "No medications scheduled" }
        return count == 1 ? "1 medication scheduled" : "\(count) medications scheduled"
    }

    private var morningBriefingSubtitle: String {
        if let last = briefingService.lastBriefingDate,
           Calendar.current.isDateInToday(last) {
            let f = DateFormatter()
            f.dateFormat = "h:mm a"
            return "Played today at \(f.string(from: last))"
        }
        let eventsToday = briefingService.eventsForDate().count
        let groundingCount = briefingService.groundingDetails.count
        if eventsToday == 0 && groundingCount == 0 {
            return "Tap to set up events and grounding details"
        }
        return "\(eventsToday) event\(eventsToday == 1 ? "" : "s") today · \(groundingCount) grounding detail\(groundingCount == 1 ? "" : "s")"
    }
            ...
            VStack(spacing: 24) {
                ...
                VStack(spacing: 12) {
                    ...
                    GlassesConnectionItem(
                        ...
                        onConnect: { Task { await wearablesService.connectGlasses() } }
                    )
                }
                ...
                VStack(spacing: 12) {
                    ...
                    ProfileActionButton(
                        ...
                        subtitle: morningBriefingSubtitle,
