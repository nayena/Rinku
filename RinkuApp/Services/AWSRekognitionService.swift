...
actor AWSRekognitionService {
    ...
    /// A single label returned by DetectLabels, with its confidence and parent categories.
    struct DetectedLabel {
        let name: String          // e.g. "Pill", "Door", "Key"
        let confidence: Float     // 0–100
        let parents: [String]     // e.g. ["Medicine", "Healthcare"]
    }
    ...
    /// Detect object/scene labels in an image using AWS Rekognition DetectLabels.
    /// - Parameters:
    ///   - image: The camera frame to analyze.
    ///   - maxLabels: Maximum number of labels to return (default 20).
    ///   - minConfidence: Minimum confidence threshold 0–100 (default 60).
    /// - Returns: Labels sorted by descending confidence.
    func detectLabels(
        in image: UIImage,
        maxLabels: Int = 20,
        minConfidence: Float = 60.0
    ) async throws -> [DetectedLabel] {
        guard AWSConfig.isConfigured else {
            throw RekognitionError.notConfigured
        }
        ...
        let response = try await makeRekognitionRequest(action: "DetectLabels", body: requestBody)

        guard let rawLabels = response["Labels"] as? [[String: Any]] else {
            return []
        }

        return rawLabels
            .compactMap { label -> DetectedLabel? in
                guard let name       = label["Name"]       as? String,
                      let confidence = label["Confidence"] as? Double else { return nil }
                let parents = (label["Parents"] as? [[String: Any]] ?? [])
                    .compactMap { $0["Name"] as? String }
                return DetectedLabel(name: name, confidence: Float(confidence), parents: parents)
            }
