...
final class AudioService: NSObject, ObservableObject, AVSpeechSynthesizerDelegate, AVAudioPlayerDelegate {
    ...
    private var pendingAudioFile: String?       // recorded .m4a to play after TTS anchor
    private var pendingUtterances: [String] = [] // queued TTS layers (drained via delegate)
    private var pendingRoutineText: String?      // Layer 4 spoken after recording ends
    ...
    /// Speak a layered recognition reminder for a loved one.
    ///
    /// - Normal path (confidence ≥ 80 or offline):
    ///   Layer 1: anchor ("That's Maria, your daughter.")
    ///   Layer 2: memoryPrompt (warm fact, if set)
    ///   Layer 3: conversationStarter (hook, if set)
    ///   Layer 4: ContextEngine routine context (if applicable)
    ///   If a voice recording exists, it replaces layers 2+3; layer 4 still fires after.
    ///
    /// - Low-confidence path (confidence < 80, excluding offline matches):
    ///   Single utterance: "You might know this person — possibly Maria, your daughter."
    @MainActor func speakRecognitionReminder(for lovedOne: LovedOne, confidence: Double = 100, isOffline: Bool = false) {
        guard isEnabled else { return }

        let anchorFormat = "tts_anchor".localized
        let anchor = String(format: anchorFormat, lovedOne.displayName, lovedOne.relationship)

        // Layer 4: routine context from ContextEngine (built regardless of path)
        let briefing = ContextEngine.shared.buildBriefing(for: lovedOne)
        let routineLayer: String? = {
            guard let active = briefing.activeRoutine else {
                guard let upcoming = briefing.upcomingRoutine,
                      let mins = briefing.minutesUntilUpcoming else { return nil }
        ...
        // Low-confidence path: single hedged utterance, no further layers
        let isLowConfidence = !isOffline && confidence < 80
        if isLowConfidence {
            let hedgeFormat = "tts_low_confidence".localized
            let hedged = String(format: hedgeFormat, lovedOne.displayName, lovedOne.relationship)
            pendingAudioFile = nil
            pendingUtterances = []
            pendingRoutineText = nil
            speak(hedged)
            return
        }

        // Normal path
        pendingRoutineText = routineLayer

        if let audioFile = lovedOne.audioFileName, !audioFile.isEmpty {
            // Recorded audio replaces layers 2+3: speak anchor, then play recording
            pendingAudioFile = audioFile
            pendingUtterances = []
            speak(anchor)
        } else {
            // TTS-only: build layer queue
            var layers: [String] = [anchor]
            if let memory = lovedOne.memoryPrompt, !memory.isEmpty {
                layers.append(memory)
            }
            if let hook = lovedOne.conversationStarter, !hook.isEmpty {
                layers.append(hook)
            }
            pendingAudioFile = nil
            speakLayers(layers)
    ...
    /// Enqueues an array of TTS utterances and speaks the first one.
    /// The delegate drains the rest one-by-one via `pendingUtterances`.
    @MainActor private func speakLayers(_ layers: [String]) {
        guard !layers.isEmpty else { return }
        pendingUtterances = Array(layers.dropFirst())
        speak(layers[0])
    }
    ...
    func speechSynthesizer(_ synthesizer: AVSpeechSynthesizer, didFinish utterance: AVSpeechUtterance) {
        // Priority: drain TTS queue → play recording → speak routine layer → done
        if !pendingUtterances.isEmpty {
            let next = pendingUtterances.removeFirst()
            Task { @MainActor in self.speak(next) }
        } else if let audioFile = pendingAudioFile {
            pendingAudioFile = nil
            playRecordedAudio(fileName: audioFile)
        } else if let routineText = pendingRoutineText {
            pendingRoutineText = nil
            Task { @MainActor in self.speak(routineText) }
        } else {
            DispatchQueue.main.async {
                self.isSpeaking = false
            }
    ...
    func audioPlayerDidFinishPlaying(_ player: AVAudioPlayer, successfully flag: Bool) {
        ...
        if let routineText = pendingRoutineText {
            pendingRoutineText = nil
            Task { @MainActor in self.speak(routineText) }
        } else {
            DispatchQueue.main.async {
